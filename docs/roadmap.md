...
| After a contract edit, `validate --write-graph` rewrites only the edges of the one marked component graph from the observed imports, sorted as `init` writes them and keeping the page's own diagram line; `graph.drift` names the command, and Archkeel's own graph was regenerated with it (AD-46) | `src/archkeel/check/validation.py`; `src/archkeel/cli/__init__.py`; `tests/test_cli.py::test_validate_write_graph_regenerates_only_the_marked_graph`; `tests/test_validation.py` |
