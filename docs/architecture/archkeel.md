...
id, a threshold that hides weak edges, and an inspector for modules and interface names. Width
encoded that weight until two costs showed: an arrow head scales with its line, so a heavy edge
grew a head that read as weight where it should read as direction, and inside a component, where
every edge is observed and the spots are crowded, the number that would have said so was the
first thing dropped. One width with the count written on it shows the same measurement exactly
rather than by eye. It is derived from the canonical observation alone, so
