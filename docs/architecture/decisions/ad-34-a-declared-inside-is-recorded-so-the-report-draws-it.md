...
`level()` already returns ([AD-24a](ad-24a-a-module-opens-the-same-way-and-its-interface-is-what.md)). A module no sub-component owns keeps a card of its own. Where the inside declares `complete_requires`, absence forbids ([AD-32](ad-32-a-component-names-what-it-requires-what-it-does-not-name-is.md)), so a crossing pair is covered or violated. Without a rule deciding that pair, it remains `observed` ([AD-24b](ad-24b-observed-is-not-undecided.md)); absence of a finding alone is not conformance. Reason: `check` declares three
