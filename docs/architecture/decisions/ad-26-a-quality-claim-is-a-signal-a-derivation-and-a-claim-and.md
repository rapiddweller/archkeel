...
inventing that number. The fact is lexical: it does not decide whether an implementation parameter
is required by an interface or safe to remove. An empty candidate list means none were recorded,
not that every parameter is read. On Archkeel itself it names nothing, because `ARG` and `RUF059`
reject such a binding at lint time: a claim that stays empty on a clean repository is working, not
missing.
