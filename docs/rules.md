...
deliberately unused binding, `self` and `cls`, and the parameters of a method that overrides another
or fills an empty stub, because something other than the body chose them.

- **Measurement:** syntactically unread candidates, and the functions examined beside them. The
  bindings set aside are not counted, because the signal records only what it names. An empty list
  means no candidates were recorded; it does not establish that every parameter is read.
