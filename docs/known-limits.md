...
FACT — EE: 13 final source reports, all Exit 0 and Diagnostics empty. The first valid report took 14.04887562499789 seconds, below 180 seconds. Their scoped Python file lists, blob hashes, source digests and report measurements were independently checked against the selected Git SHAs. There are also two preserved setup failures for `df869a07`: both Exit 2, `kind=parse_error`, `subject=producer` (archive prefix omitted), `unknown_claim` ends in the following contract errors; remedy for both is `"Repair the producer failure and run report again."`
...
UNKNOWN — Exact total EE invocations and one setup Diagnostic cannot be reconstructed: overlapping temporary runner sessions reused filenames for `c799b024`. Retained process metadata says Exit 2 / 0.10166962500079535 seconds, while its associated result says Exit 0 and has no Diagnostics. Their pairing is invalid. That attempt is excluded from final comparisons and timings; its original Exit-2 JSON is unavailable. A later complete report for that SHA is independently source-verified. The retained evidence contains 16 complete result files and one unmatched Exit-2 metadata record; this does not establish an exact invocation count.

FACT — Median report runtime: Repo #2 0.497853 seconds across all 23 invocations; EE 14.267670 seconds across the 13 final source reports only. No median over all EE attempts is claimed.
