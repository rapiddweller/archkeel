...
from codekeel.ir.measurements import Measurements
from codekeel.ir.model import Diagnostic, Observation, Record, RunResult


def _asset(name: str) -> bytes:
    return files("codekeel.check").joinpath("assets", name).read_bytes()
...
def render_html(
    ...
    *,
    ...
    css = _asset("codekeel-report.css").decode("utf-8")
    dark_logo = _data_uri("codekeel-logo-dark.svg", "image/svg+xml")
    light_logo = _data_uri("codekeel-logo-light.svg", "image/svg+xml")
    mark = _data_uri("codekeel-mark.svg", "image/svg+xml")
...
<head>
  ...
  <title>Codekeel report · {_text(repository)}</title>
  <style>{css}</style>
</head>
<body>
  <header class="report-header">
    <img class="report-logo report-logo-dark" src="{dark_logo}" alt="Codekeel">
    <img class="report-logo report-logo-light" src="{light_logo}" alt="Codekeel">
  ...
  <main class="report-shell">
    ...
    <footer class="report-footer">Codekeel · deterministic architecture evidence</footer>
  </main>
</body>
</html>
"""
