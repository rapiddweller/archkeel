...
def _dynamic_call_signals(
    calls: Sequence[RawRecord], evidence: dict[str, RawEvidence]
) -> list[RawRecord]:
    dynamic_targets = {
    ...
    for call in calls:
        ...
        items.append(
            classified(
                ...
                evidence_ids=[value for value in call["evidence_ids"] if value in evidence],
                fact_ids=[call["id"]],
                data={
...
def _boundary_annotation_signals(
    imports: Sequence[RawRecord], symbols: Sequence[RawRecord]
) -> list[RawRecord]:
