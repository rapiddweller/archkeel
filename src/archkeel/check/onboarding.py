...
def draft_contract(
    ...
    return ArchitectureContract(CONTRACT_SCHEMA_VERSION, components, tuple(rules)), edges, sizes
...
def architecture_document(
    ...
    rows = "\n".join(
        f"| `{component.label}` | `{component.packages[0]}` | {by_label[component.label].modules} "
        f"| {by_label[component.label].inner_edges} | TODO: describe the responsibility. |"
    ...
    return (
        ...
        "| Component | Package | Modules | Inner edges | Responsibility |\n|---|---|---|---|---|\n"
...
def run_init(
    ...
    *,
    ...
    result = RunResult(
        ...
        draft_sizes=tuple(
            DraftedComponentSize(metric.scope, metric.modules, metric.inner_edges)
            for metric in sizes
        ),
    )
    return result, files
