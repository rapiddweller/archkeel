...
"""Validated Pledge repository configuration."""
...
from pledge.check.git import read_blob
from pledge.check.ports import ScanConfig


class ConfigError(ValueError):
    """Raised when pledge.toml is invalid or unsafe for a repository."""
...
def parse_config(payload: bytes) -> ScanConfig:
    ...
    try:
        ...
        raise ConfigError(f"invalid pledge.toml: {exc}") from exc
...
def load_config(root: Path) -> ScanConfig:
    """Load pledge.toml and validate paths against the repository filesystem."""
    repository = root.resolve()
    config_path = repository / "pledge.toml"
    try:
        config_path.resolve().relative_to(repository)
        payload = config_path.read_bytes()
    except ValueError as exc:
        raise ConfigError("pledge.toml escapes repository root") from exc
    except OSError as exc:
        raise ConfigError(f"cannot read pledge.toml: {exc}") from exc
...
def load_check_config(root: Path, baseline: str, head: str) -> ScanConfig:
    ...
    payload = read_blob(root, baseline, "pledge.toml")
    if payload != read_blob(root, head, "pledge.toml"):
        raise ConfigError("candidate changed accepted policy input: pledge.toml")
