...
def test_html_report_rendered_from_architecture_json_alone_shows_agent_decisions(
    ...
    # 39 rules (AD-11, issue #47), 8 declared public lists and the inside's 3 requires entries
    # (AD-50).
    assert result.agent_decisions == (1, 50)
    assert "1 of 50 decisions made by the agent" in report_summary(result).sentence
    ...
    assert "1 of 50 decisions made by the agent, awaiting the architect." in page
